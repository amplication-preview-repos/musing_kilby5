/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  IsDate,
  ValidateNested,
} from "class-validator";
import { Type } from "class-transformer";
import { UserWhereUniqueInput } from "../../user/base/UserWhereUniqueInput";
import { RatingUpdateManyWithoutMoviesInput } from "./RatingUpdateManyWithoutMoviesInput";
import { CommentUpdateManyWithoutMoviesInput } from "./CommentUpdateManyWithoutMoviesInput";

@InputType()
class MovieUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  title?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  releaseDate?: Date | null;

  @ApiProperty({
    required: false,
    type: () => UserWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UserWhereUniqueInput)
  @IsOptional()
  @Field(() => UserWhereUniqueInput, {
    nullable: true,
  })
  user?: UserWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => RatingUpdateManyWithoutMoviesInput,
  })
  @ValidateNested()
  @Type(() => RatingUpdateManyWithoutMoviesInput)
  @IsOptional()
  @Field(() => RatingUpdateManyWithoutMoviesInput, {
    nullable: true,
  })
  ratings?: RatingUpdateManyWithoutMoviesInput;

  @ApiProperty({
    required: false,
    type: () => CommentUpdateManyWithoutMoviesInput,
  })
  @ValidateNested()
  @Type(() => CommentUpdateManyWithoutMoviesInput)
  @IsOptional()
  @Field(() => CommentUpdateManyWithoutMoviesInput, {
    nullable: true,
  })
  comments?: CommentUpdateManyWithoutMoviesInput;
}

export { MovieUpdateInput as MovieUpdateInput };
